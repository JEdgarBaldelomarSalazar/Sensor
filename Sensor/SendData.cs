...
namespace Sensor
{
    public class SendData : WebSocketBehavior
    {
        ...
        private readonly int _convertMs = 1000;
        private int _period;
        private Random _rnd;
        public SendData()
        {
        ...
        protected override void OnMessage(MessageEventArgs e)
        {
            ...
            if (int.TryParse(e.Data, out int period))
            {
                _period = period;

                Task<double> tempTask = Generate();
                tempTask.ContinueWith(task =>
                {
                    string tempString = task.Result.ToString();

                    Sessions.Broadcast(tempString);
                });
            }

        }
        private async Task<double> Generate()
        {
